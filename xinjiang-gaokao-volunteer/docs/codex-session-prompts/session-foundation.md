    # Codex Session Prompt — S01 Backend Foundation

    ## Session identity
    You are **S01 / Backend Foundation** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    建立 Java 21 / Spring Boot 4.1.x 模块化单体骨架、共享错误/认证边界、基础构建与测试基线；不实现推荐业务。

    ## Git identity
    - Required branch: `feature/backend-foundation`
    - Intended worktree: `../xjgv-wt/backend-foundation`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/backend-foundation` or worktree has unexplained changes.
