    # Codex Session Prompt — S03 Core Recommendation

    ## Session identity
    You are **S03 / Core Recommendation** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    实现候选人版本画像读取、Hard Rule 过滤、RecommendationRun、幂等与 quota 事务语义；AI 仅留边界。

    ## Git identity
    - Required branch: `feature/recommendation-core`
    - Intended worktree: `../xjgv-wt/recommendation-core`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/recommendation-core` or worktree has unexplained changes.

    ## Owned scope
    Writable owner scope:
    - `backend/src/main/java/com/xjgaokao/{candidate,policy,admission,recommendation,membership}/`
...
- Explicitly not loaded: `ai-explanation`
- Explicitly not loaded: `payment-safety`
