    # Codex Session Prompt — S06 Data Pipeline Foundation

    ## Session identity
    You are **S06 / Data Pipeline Foundation** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    建立 Python 3.12+ 离线数据流水线骨架、来源/版本/质量校验接口；不提供在线 API。

    ## Git identity
    - Required branch: `feature/data-pipeline-foundation`
    - Intended worktree: `../xjgv-wt/data-pipeline-foundation`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/data-pipeline-foundation` or worktree has unexplained changes.
...
- Explicitly not loaded: `recommendation-engine`
