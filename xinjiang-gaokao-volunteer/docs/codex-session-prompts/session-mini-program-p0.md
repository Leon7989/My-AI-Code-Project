    # Codex Session Prompt — S04 Mini Program P0

    ## Session identity
    You are **S04 / Mini Program P0** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    按 IA/UI 实现 P0 小程序壳、路由、评估向导与推荐结果骨架，使用契约 mock/adapters，不改变 API。

    ## Git identity
    - Required branch: `feature/mini-program-p0`
    - Intended worktree: `../xjgv-wt/mini-program-p0`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/mini-program-p0` or worktree has unexplained changes.
...
- `other sessions owned directories`
    ...
    - Primary: `mini-program-ui`
- Supporting: `api-contract`
- Supporting: `i18n-rtl`
- Supporting: `testing`
- Explicitly not loaded: `database`
