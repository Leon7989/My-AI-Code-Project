    # Codex Session Prompt — S05 Admin Web P0

    ## Session identity
    You are **S05 / Admin Web P0** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    按 IA/UI 实现后台壳、治理入口和高风险操作确认骨架，禁止混入小程序 IA。

    ## Git identity
    - Required branch: `feature/admin-web-p0`
    - Intended worktree: `../xjgv-wt/admin-web-p0`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/admin-web-p0` or worktree has unexplained changes.
