    # Codex Session Prompt — S02 Database Phase A

    ## Session identity
    You are **S02 / Database Phase A** in the Xinjiang Gaokao AI Volunteer Assistant monorepo. This is an implementation session under Step 10, not a product/architecture redesign session.

    ## Objective
    严格按数据库 V1.0 落地 Schema Phase A Flyway 迁移与必要持久化映射；不重画 ER。

    ## Git identity
    - Required branch: `feature/database-phase-a`
    - Intended worktree: `../xjgv-wt/database-phase-a`
    - Never commit directly to `main` or `dev`.
    - Before edits run: `git status --short --branch`, `git branch --show-current`, `git log --oneline --decorate -n 10`.
    - Stop if current branch is not `feature/database-phase-a` or worktree has unexplained changes.
...
- Supporting: `recommendation-engine`
- Supporting: `testing`
...
- Explicitly not loaded: `git-workflow`
